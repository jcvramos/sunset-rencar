...
const sidebarOpen = ref(false);   // móvil: abrir/cerrar
const collapsed   = ref(false);   // desktop: modo compacto (solo íconos)
...
<template>
    <div class="min-h-screen flex" style="background:#f5f3ef; font-family:'Inter',sans-serif;">
        ...
        <!-- ========= SIDEBAR ========= -->
        <aside
            class="fixed lg:static inset-y-0 left-0 z-40 transition-all duration-200 flex flex-col"
            :class="[
            ...
            <div class="px-3 py-4 flex items-center gap-2"
                 ...
                 style="border-bottom:1px solid rgba(251,191,36,0.10);">

                <Link :href="route('dashboard')" class="shrink-0 flex items-center gap-2">
                    <img src="/img/logo.png" alt="Sunset" class="h-9 w-auto"
                         style="filter: drop-shadow(0 0 10px rgba(251,191,36,0.6)) brightness(1.1);"/>
                    <div v-if="!collapsed">
                        <p class="text-sm font-bold leading-tight" style="color:#fbbf24;">Sunset RenCar</p>
                        <p class="text-[10px] uppercase tracking-wider leading-tight" style="color:rgba(251,191,36,0.4);">Panel operativo</p>
                    </div>
                </Link>
                ...
                <button v-if="!collapsed"
                        ...
                        class="hidden lg:flex p-1.5 rounded transition-colors"
                        title="Colapsar menú"
                        ...
                        onmouseout="this.style.background='transparent';this.style.color='rgba(251,191,36,0.5)'">
                    <svg class="h-4 w-4" stroke="currentColor" fill="none" viewBox="0 0 24 24" stroke-width="2">
            ...
            <button v-if="collapsed"
                    ...
                    class="hidden lg:flex justify-center py-2 mx-2 mt-2 rounded transition-colors"
                    title="Expandir menú"
                    style="color:rgba(251,191,36,0.5);"
                    onmouseover="this.style.background='rgba(251,191,36,0.07)';this.style.color='rgba(251,191,36,0.9)'"
                    onmouseout="this.style.background='transparent';this.style.color='rgba(251,191,36,0.5)'">
            ...
            <nav class="flex-1 overflow-y-auto px-2 py-4 space-y-4">
                <div v-for="group in navGroups" :key="group.title">
                    <p v-if="!collapsed" class="px-3 text-[10px] uppercase tracking-wider mb-1.5"
                       style="color:rgba(251,191,36,0.30);">
                        {{ group.title }}
                    </p>
                    <div v-else class="border-t mx-2 my-2" style="border-color:rgba(251,191,36,0.10);"></div>
                    ...
                    <Link
                        ...
                        class="flex items-center gap-2.5 rounded-lg text-sm font-medium transition-all mb-0.5"
                        :class="collapsed ? 'px-0 py-2.5 justify-center' : 'px-3 py-2'"
                        :style="route().current(nav.route)
                            ? 'background:rgba(251,191,36,0.15); color:#fbbf24;' + (collapsed ? '' : ' box-shadow: inset 3px 0 0 #fbbf24;')
                    ...
                    >
                        ...
                        <span v-if="!collapsed">{{ nav.label }}</span>
                    </Link>
                </div>
            </nav>

            <!-- Usuario al fondo -->
            <div class="px-2 py-3" style="border-top:1px solid rgba(251,191,36,0.10);">
                <Dropdown align="left" :width="collapsed ? '52' : '48'">
                    <template #trigger>
                        <button class="w-full flex items-center gap-2.5 rounded-lg transition-all"
                                :class="collapsed ? 'px-0 py-2 justify-center' : 'px-2 py-2'"
                                :title="collapsed ? $page.props.auth.user.name : ''"
                            ...
                            <svg v-if="!collapsed" class="h-3.5 w-3.5 opacity-60" viewBox="0 0 20 20" fill="currentColor">
